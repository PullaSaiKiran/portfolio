...
import { Mail, Phone, MapPin, Github, Printer, Info } from 'lucide-react';
...
const Resume: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      {/* Controls */}
      <div className="mb-8 flex flex-col sm:flex-row justify-between items-center bg-slate-800/50 p-4 rounded-xl border border-slate-700 print:hidden gap-4">
        <div className="flex items-center text-slate-300 text-sm">
          <Info className="w-4 h-4 mr-2 text-blue-400" />
          <span>Select <b>"Save as PDF"</b> in the destination dropdown of the print dialog.</span>
        </div>
        <button 
          onClick={handlePrint}
          className="flex items-center px-6 py-3 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-lg transition-all shadow-lg shadow-emerald-500/20 active:scale-95"
        >
          <Printer className="w-5 h-5 mr-2" /> Download / Save as PDF
        </button>
      </div>
      ...
      <div className="resume-container bg-white text-slate-900 w-full max-w-[210mm] mx-auto p-8 sm:p-12 shadow-2xl print:shadow-none print:w-full print:max-w-none">
